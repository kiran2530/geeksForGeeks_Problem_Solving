/*
 * march-11-2024 : Count pairs Sum in matrices
...
public class march_11_24 {
    public static int countPairs(int mat1[][], int mat2[][], int n, int x) {
        ...
        for(int i=0; i<n; i++) {
            for(int j=0; j<n; j++) {
                int a = mat1[i][j];
                int i2=0,j2=n-1;
                while(i2<n && a+mat2[i2][j2] < x) 
                    i2++;
                if(i2<n){
                    while(j2>=0 && mat2[i2][j2]+a > x) 
                        j2--;
                    if(j2>=0 && mat2[i2][j2]+a==x) 
                        count++;
                }
            }
        }
    ...
    public static void main(String[] args) {
        int n = 3, x = 21;
        int[][] mat1 = {{1, 5, 6}, {8, 10, 11}, {15, 16, 18}}, mat2 = {{2, 4, 7}, {9, 10, 12}, {13, 16, 20}};
